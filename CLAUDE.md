...
- `scoop-daily.ps1` — 交互式 scoop 更新（版本分级 + 选择更新 + 进程冲突检测）
...
- scoop update 进程冲突检测：当软件进程正在运行时，scoop 返回退出码 0 但输出 "Running process detected, skip updating"，脚本会识别此情况并单独统计为"跳过"而非"成功"。
